...
<html>
    ...
    <?php
        ...
        if ($search) {
            ...
            $products = mysqli_query($con, "SELECT * FROM `product` WHERE category = 'Xiaomi' " . $orderConditon . " LIMIT " . $item_per_page . " OFFSET " . $offset);
            $totalRecords = mysqli_query($con, "SELECT COUNT(*) as total FROM `product` WHERE category = 'Xiaomi'");
        }
