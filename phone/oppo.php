...
<html>
    ...
    <?php
        ...
        if($search){
            ...
            $products = mysqli_query($con, "SELECT * FROM `product` where category = 'Oppo' ORDER BY `id` ASC  LIMIT " . $item_per_page . " OFFSET " . $offset);
             $totalRecords = mysqli_query($con, "SELECT COUNT(*) as total FROM `product` where category = 'Oppo'");
         }
